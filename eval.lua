...
opt = {
  ...
  gpu = 1,
  cudnn = 1,
  model = 'checkpoints/regression/net.t7',
  ntest = math.huge,
  randomize = 0,
  cropping = 'center',
  data_root = '/do_not_store/ananth/dataset/train',
  data_list = '/do_not_store/ananth/dataset/train/val_numbers.txt',
...
local predicted = torch.DoubleTensor()
...
for iter = 1, maxiter do
  ...
  local aa = angle:forward{label, output}
  predicted = (iter==1) and aa or torch.cat(predicted, aa, 1)

  for i=1,opt.batchSize do
    print(('%s: Eval GT: %s predicted: %s '):format(opt.model,tostring(data_label[i]),tostring(predicted[i])))
...
torch.save('result.t7',predicted:double())
